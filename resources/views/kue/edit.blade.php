@extends('layouts.app')

@section('content')
@include('sweetalert::alert')

<div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md mt-8">
    <h2 class="text-xl font-bold mb-4">Edit Kue</h2>

    @if ($errors->any())
        <script>
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
                text: 'Silakan periksa kembali form Anda!',
            });
        </script>
    @endif

    <form action="{{ route('kue.update', $kue->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="nama_kue" class="block text-gray-700">Nama Kue</label>
            <input type="text" id="nama_kue" name="nama_kue" value="{{ old('nama_kue', $kue->nama_kue) }}"
                   class="w-full border-gray-300 rounded-lg p-2 focus:ring focus:ring-blue-200">
            @error('nama_kue')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

        <div class="mb-4">
            <label for="harga" class="block text-gray-700">Harga</label>
            <input type="number" id="harga" name="harga" min="1" value="{{ old('harga', $kue->harga) }}"
                   class="w-full border-gray-300 rounded-lg p-2 focus:ring focus:ring-blue-200">
            @error('harga')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

        <div class="mb-4">
            <label for="stok" class="block text-gray-700">Stok</label>
            <input type="number" id="stok" name="stok" min="1" value="{{ old('stok', $kue->stok) }}"
                   class="w-full border-gray-300 rounded-lg p-2 focus:ring focus:ring-blue-200">
            @error('stok')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

        <div class="mb-4">
            <label for="kategori" class="block text-gray-700">Kategori</label>
            <select name="kategori_id" id="kategori" class="w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-300">
                <option value="">Pilih Kategori</option>
                @foreach($kategoris as $kategori)
                    <option value="{{ $kategori->id }}" {{ old('kategori_id', $kue->kategori_id) == $kategori->id ? 'selected' : '' }}>
                        {{ $kategori->nama_kategori }}
                    </option>
                @endforeach
            </select>
            @error('kategori_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
            Simpan Perubahan
        </button>

        <a href="{{ route('dashboard') }}" class="ml-2 text-gray-600 hover:text-gray-900">Batal</a>
    </form>
</div>
@endsection
